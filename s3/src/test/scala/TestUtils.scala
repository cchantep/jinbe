package io.github.cchantep.jinbe.s3.tests
...
import io.github.cchantep.jinbe.s3.{ S3, WSS3 }
...
object TestUtils {
  ...
  lazy val system: ActorSystem = ActorSystem("jinbe-s3-tests")
    ...
    if (inited) {
      ...
      def storageCleanup(st: ObjectStorage): Future[Unit] =
        st.buckets
          .collect[List]()
          .flatMap(bs =>
            Future.sequence(bs.filter(_.name startsWith "jinbe-test-").map {
