...
import tests.jinbe.JinbeMatchers

import org.specs2.concurrent.{ ExecutionEnv => EE }
import org.specs2.specification.core.Fragment

trait S3Spec extends JinbeMatchers {
  spec: org.specs2.mutable.Specification =>

  import TestUtils.withMatEx

  def s3Suite(
      s3f: => io.github.cchantep.jinbe.s3.WSS3,
      bucketName: String,
      objName: String
    )(implicit
      m: Materializer
    ...
    "paginate the bucket objects" in withMatEx { implicit ee: EE =>
      ...
      type StructType = {
        def list(
          ...
          )(andThen: String => Source[Object, akka.NotUsed]
          )(implicit
            m: Materializer
    ...
    "Use correct toString format on object" in {
      storage
        .bucket("bucketName")
        .obj("objectName")
        .toString must_== "WSS3ObjectRef(bucketName, objectName)"
    }

    s"write and delete a file with object name containing a slash" in withMatEx {
      implicit ee: EE =>
