...
import tests.jinbe.{ StorageCommonSpec, VersioningCommonSpec }

import org.specs2.concurrent.{ ExecutionEnv => EE }

final class S3CephSpec
  ...
  "Client" should {
    val bucketName = s"jinbe-test-${random.nextInt().toString}"
    ...
    s"Write file in $bucketName bucket using multipart" >> {
      "with the default maximum" in withMatEx { implicit ee: EE =>
        ...
        def body: Source[Array[Byte], akka.NotUsed] =
      ...
      "with a maximum of 2 parts instead of 3" in withMatEx { implicit ee: EE =>
        ...
        (body runWith upload).flatMap(_ => obj.exists).aka("upload") must beTrue
          .await(2, /*10*/ 45.seconds) and {
          partCount must_== 2
        }
      }
    }
