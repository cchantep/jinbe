...
import io.github.cchantep.jinbe.exception.ObjectNotFoundException
import io.github.cchantep.jinbe.s3.QueryParameters._
import io.github.cchantep.jinbe.ws.{ ContentMD5, Successful }
    ...
    withContentTypeHeader(req, contentType).put(bytes).flatMap {
      ...
      case response @ Successful(_) =>
        Future.failed[String](
          new IllegalStateException(
            s"Response for the upload [$bucket/$name, $uploadId, part: ${partNumber.toString}] did not include an ETag header: ${response.headers.mkString("{", ",", "}")}."
          )
        )
