...
import io.github.cchantep.jinbe.ws.Successful
...
final class WSS3VersionedObjectRef(
  ...
  def headers(
    ...
    def req = storage.request(
      Some(bucket),
      Some(name),
      query = Some(s"versionId=$versionId"),
  ...
  final private[s3] case class WSS3DeleteRequest(
    ...
    @inline
    private def isDeleteMarker(version: VersionedObject) =
      version.size.bytes == -1

    private def markersToDelete(
      )(implicit
        m: Materializer
    ...
    private def deleteSingle(
      ...
      storage
        .request(
          Some(bucket),
          Some(v.name),
          query = Some(s"versionId=${v.versionId}"),
          requestTimeout = requestTimeout
        )
        .delete()
        .flatMap {
          ...
          case response => {
            ...
            errorHandler(response) match {
              case VersionNotFoundException(_, _, _) if ignoreExists =>
                Future.successful(
                  logger.info(s"Version $bucket/${v.name}/${v.versionId} was not found when deleting. (Success)")
                )
    ...
    private def multiDeleteSimulated(
        versions: Seq[VersionedObject]
      )(implicit
        m: Materializer
      ): Future[Unit] = {
      implicit val ec: ExecutionContext = m.executionContext
