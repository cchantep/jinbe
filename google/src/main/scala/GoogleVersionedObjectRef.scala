...
import io.github.cchantep.jinbe.{ ByteRange, VersionedObjectRef }
import io.github.cchantep.jinbe.exception.VersionNotFoundException
      ...
      case js =>
        Future.failed[Map[String, Seq[String]]](
          new IllegalStateException(
            s"Could not get the headers of version $versionId of the object $name in the bucket $bucket. JSON response: ${Json stringify js}"
          )
        )
    }
  }.recoverWith(
    ErrorHandler.ofVersionToFuture(
      s"Could not get the headers of version $versionId of the object $name in the bucket $bucket",
      ref
    )
  )
  ...
  private[google] final class GoogleGetRequest() extends GetRequest {
    ...
    def apply(
      ...
      Source.fromFutureSource(
        Future {
          val req =
            gt.client.objects().get(bucket, name).setGeneration(generation)
