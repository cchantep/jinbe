...
import io.github.cchantep.jinbe.URIProvider
import io.github.cchantep.jinbe.google.GoogleTransport
...
final class GoogleTransportSpec extends Specification {
  ...
  private val application =
    s"jinbe-tests-${System.identityHashCode(this).toString}"

  "Factory using URI" should {
    "succeed" >> {
      "when given a proper uri as String" in {
        GoogleTransport(
          s"google:classpath://$filename?application=$application&projectId=$projectId"
      ...
      "with optional request timeout in URI" in {
        val uri = new URI(
          s"google:classpath://$filename?application=$application&projectId=$projectId&requestTimeout=123"
        )

        GoogleTransport(uri) must beSuccessfulTry[GoogleTransport].like {
          case transport =>
            transport.requestTimeout must beSome[Long].which {
      ...
      "with GZip disabled in URI" in {
        val uri = new URI(
          s"google:classpath://$filename?application=$application&projectId=$projectId&disableGZip=true"
        )
    ...
    "fail" >> {
      "when the provider fail" in {
        ...
        GoogleTransport(exception) must beFailedTry.withThrowable[Exception](
          "foo"
        )
      }
      ...
      "with wrong scheme" in {
        GoogleTransport(
          s"google:wrong://$filename?application=$application&projectId=$projectId"
        ) must beFailedTry.withThrowable[MalformedURLException]
      }

      "without scheme prefix" in {
        GoogleTransport(
          s"classpath://$filename?application=$application&projectId=$projectId"
        ) must beFailedTry.withThrowable[IllegalArgumentException]
      }

      "when given a uri without application parameter" in {
        GoogleTransport(
          s"google:classpath://$filename?projectId=$projectId"
        ) must beFailedTry.withThrowable[IllegalArgumentException]
      }

      "when given a uri without projectId parameter" in {
        GoogleTransport(
          s"google:classpath://$filename?application=$application"
        ) must beFailedTry.withThrowable[IllegalArgumentException]
      }

      "with invalid request timeout in URI" in {
        val uri = new URI(
          s"google:classpath://$filename?application=$application&projectId=$projectId&requestTimeout=AB"
        )
      ...
      "with value for 'disableGZip' in URI" in {
        val uri = new URI(
          s"google:classpath://$filename?application=$application&projectId=$projectId&disableGZip=Foo"
        )
