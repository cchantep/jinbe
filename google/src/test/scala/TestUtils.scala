package io.github.cchantep.jinbe.google.tests
...
import io.github.cchantep.jinbe.google.{ GoogleStorage, GoogleTransport, WS }
...
object TestUtils {
  ...
  lazy val system: ActorSystem = ActorSystem("jinbe-google-tests")
  ...
  lazy val configUri: String = {
    ...
    val application = s"jinbe-tests-${System.identityHashCode(this).toString}"
  ...
  def close(): Unit = {
    if (inited) {
      ...
      def storageCleanup(st: ObjectStorage) = st.buckets
        .collect[List]()
        .flatMap(bs =>
          Future.sequence(bs.filter(_.name startsWith "jinbe-test-").map { b =>
