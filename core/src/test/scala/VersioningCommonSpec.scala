...
import io.github.cchantep.jinbe.{
  BucketRef,
  BucketVersioning,
  ObjectStorage,
  ObjectVersioning,
...
trait VersioningCommonSpec extends JinbeMatchers with ErrorCommonSpec {
  self: org.specs2.mutable.Specification =>
  import tests.jinbe.StreamUtils._
   ...
   */
  protected def supportsDeleteMarkers = true
  ...
  def commonVersioningTests(
    ...
    val bucketName = s"jinbe-test-versioning-${random.nextInt().toString}"
    ...
    s"support versioning for $bucketName bucket" >> {
      ...
      "to return all object versions" in {
        bucket.versioning must beSome[BucketVersioning].which { vbucket =>
          ...
          {
            vbucket.setVersioning(enabled = true).map(_ => true) must beTrue
              .await(1, 3.seconds)
            ...
            createObject(bucket, objectName, "hello") must beTrue.await(
              1,
              3.seconds
            )
            ...
            createObject(bucket, objectName, "hello world") must beTrue.await(
              2,
              3.seconds
            )
          } and {
            vbucket.versionedObjects
              ...
              case l @ (a :: b :: Nil) =>
                {
                  a.name must_=== b.name and (l must contain(firstVersion))
                } and {
                  l.find(_ != firstVersion)
                    .aka("2nd version") must beSome[VersionedObject].which {
                    ver =>
      ...
      "to keep all object versions when versioning is disabled" in {
        bucket.versioning must beSome[BucketVersioning].which { vbucket =>
          ...
          {
            createObject(bucket, objectName, "hello again !") must beTrue.await(
              2,
              3.seconds
            )
          } and {
      ...
      "to delete recursively non-empty versioned bucket" in {
        bucket must existsIn(storage, 2, 3.seconds) and {
          bucket.versioning must beSome[BucketVersioning].which { vbucket =>
            ({
              vbucket
                .setVersioning(enabled = true)
                .map(_ => {}) must beTypedEqualTo({})
                .setMessage("versioning disabled before")
                .await(2, 5.seconds)
              ...
              vbucket.versionedObjects
                .collect[List]() must beEmpty[List[VersionedObject]]
                .setMessage("!contain object")
                .await(2, 5.seconds)
            } and {
              bucket.delete.recursive() must beTypedEqualTo({})
                .setMessage("deleted #2")
                .await(2, 5.seconds)

            } and {
              bucket must notExistsIn(storage, rwConsistencyRetry, 3.seconds)
                .setMessage("!exists #2")
            }
          }
        }
              ...
              case ver :: Nil =>
                {
                  ...
                  bucket
                    .obj(objectName)
                    .get()
                    .runWith(consume) must beTypedEqualTo("hello world").await(
                    2,
                    5.seconds
                  )
                  ...
                  vbucket.versionedObjects
                    .collect[List]()
                    .map(_.size) must beTypedEqualTo(2).await(1, 3.seconds)
                  ...
                  vbucket
                    .obj(objectName, ver.versionId)
                    .get()
                    .runWith(consume) must beTypedEqualTo("hello").await(
                    2,
                    5.seconds
                  )
                } and {
      ...
      "to delete specific version by reference" in {
        bucket.versioning must beSome[BucketVersioning].which { vbucket =>
          ...
          val versions = Promise[(VersionedObject, VersionedObject)]()

          eventually(3, 3.seconds) {
            vbucket.versionedObjects
              .collect[List]()
              .map(_.sortBy(_.size.bytes) must beLike[List[VersionedObject]] {
                case a :: b :: Nil =>
                  versions.success(a -> b)

                  a.name must_=== b.name
              })
              .await(1, 3.seconds)
          } and {
            versions.future must beLike[(VersionedObject, VersionedObject)] {
              case (version1, version2) =>
                val v1 = vbucket.obj(version1.name, version1.versionId)
                val v2 = vbucket.obj(version2.name, version2.versionId)

                v1.get().runWith(consume) must beTypedEqualTo("hello").await(
                  2,
                  5.seconds
                ) and {

                  v2.get().runWith(consume) must beTypedEqualTo("hello world")
                    .await(2, 5.seconds)

                } and {
                  obj.get().runWith(consume) must beTypedEqualTo("hello world")
                    .await(2, 5.seconds)
                } and {
                  v1.delete() must beTypedEqualTo({}).await(2, 5.seconds)
                } and {
                  v1 must notExistsIn(vbucket, 2, 3.seconds)
                } and {
                  v2 must existsIn(vbucket, 2, 3.seconds)
                } and {
      ...
      "to build ObjectVersionRef from bucket or object" in {
        bucket.versioning must beSome[BucketVersioning].which { vbucket =>
          vbucket.versionedObjects
            .collect[List]() must beLike[List[VersionedObject]] {
            case v :: Nil =>
              v.versionId must not(beEmpty[String]) and {
                bucket
                  .obj(v.name)
      ...
      "to list versions of a specific object" in {
        bucket.versioning must beSome[BucketVersioning].which { vbucket =>
          ({
            ...
            createObject(bucket, testObjName, "test v1") must beTrue.await(
              2,
              3.seconds
            )

          } and {
            createObject(bucket, testObjName, "test v1.1") must beTrue.await(
              2,
              3.seconds
            )
