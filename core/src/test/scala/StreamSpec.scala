...
final class StreamSpec(
  ...
  private implicit lazy val system: akka.actor.ActorSystem =
    akka.actor.ActorSystem("jinbe-core-tests")
  ...
  "Consumer" should {
    "try to take at least up to 5 bytes" >> {
      def consumer5: Sink[ByteString, Future[Seq[Chunk]]] =
        Streams.consumeAtLeast(Bytes(5)).toMat(Sink.seq[Chunk]) { (_, chunk) =>
          chunk
        }
    ...
    "try to take at most up to 5 bytes" >> {
      def consumer5: Sink[ByteString, Future[Seq[Chunk]]] =
        Streams.consumeAtMost(Bytes(5)).toMat(Sink.seq[Chunk]) { (_, chunk) =>
          chunk
        }
      ...
      "from a single part of more than 5 bytes" in assertAllStagesStopped {
        withSource(7) {
          _.runWith(consumer5).map(_.toList) must beLike[List[Chunk]] {
            case _1 :: _2 :: Nil =>
              _1 must_== Chunk(ByteString(1, 1, 1, 1, 1)) and (_2 must_== Chunk
                .last(ByteString(1, 1)))
        ...
        withSource(5) { part1 =>
          withSource(3, 2) { part2 =>
            (part1 ++ part2)
              ...
              case _1 :: _2 :: Nil =>
                _1 must_== Chunk(
                  ByteString(1, 1, 1, 1, 1)
                ) and (_2 must_== Chunk.last(ByteString(2, 2, 2)))
            }.await(1, timeout)
          }
        }
      }
  ...
  def withSource(
      chunkSz: Int = 0,
      by: Byte = 1
    )(f: Source[ByteString, akka.NotUsed] => MatchResult[Future[Seq[Chunk]]]
    ): MatchResult[Future[Seq[Chunk]]] = {
