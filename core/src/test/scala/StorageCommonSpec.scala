...
import io.github.cchantep.jinbe.{ ByteRange, ObjectRef, ObjectStorage }

import org.specs2.concurrent.ExecutionEnv
import org.specs2.matcher.MatchResult
import org.specs2.specification.core.Fragment

trait StorageCommonSpec extends JinbeMatchers with ErrorCommonSpec {
  ...
  def commonTests(
    ...
    "Create & delete buckets" in assertAllStagesStopped {
      val name = s"jinbe-test-create-${random.nextInt().toString}"
    ...
    "Creating & deleting non-empty buckets" in assertAllStagesStopped {
      val name = s"jinbe-test-nonempty-${random.nextInt().toString}"
      val bucket = storage.bucket(name)
    ...
    "Write and copy file" in assertAllStagesStopped {
      ...
      // See https://github.com/cchantep/jinbe/pull/23
    ...
    "Delete on buckets successfully ignore when not existing" in {
      val bucket =
        storage.bucket(s"jinbe-test-testignore-${random.nextInt().toString}")
    ...
    "Versioning feature should be consistent between buckets and objects" in {
      ...
      val obj =
        bucket.obj(s"jinbe-test-versioning-obj-${random.nextInt().toString}")
