...
import io.github.cchantep.jinbe.tests.TestUtils.{
  bucketAlreadyExists,
  bucketNotEmpty,
  bucketNotFound,
  objectNotFound,
...
trait ErrorCommonSpec extends JinbeMatchers {
  ...
  def errorCommonTests(
    ...
    val nonExistingBucket = storage.bucket(
      s"jinbe-test-non-existing-bucket-${random.nextInt().toString}"
    )

    val existingBucket =
      storage.bucket(s"jinbe-test-existing-bucket-${random.nextInt().toString}")
    ...
    def put(
      ...
      Source
        .fromIterator(() => Iterator.fill(repeat)(content))
        .runWith(obj.put[Array[Byte]])
        .map(_ => {})
    }
    ...
    "Error handling" >> {
      ...
      "Setup" in assertAllStagesStopped {
        {
          ...
          put(existingObj, "hello".getBytes("UTF-8")) must beDone.await(
            2,
            3.seconds
          )
      ...
      "BucketNotFoundException should be thrown when" >> {
        "Listing a non-existing bucket" in assertAllStagesStopped {
          nonExistingBucket.objects.collect[List]() must throwA(
            bucketNotFound(nonExistingBucket)
          ).await(2, 5.seconds)
        }
          ...
          "a simple 'hello'" in assertAllStagesStopped {
            eventually(2, 3.seconds) {
              put(objOfNonExistingBucket, "hello".getBytes("UTF-8"))
                .aka("put") must throwA(bucketNotFound(nonExistingBucket))
                .await(1, 3.seconds)
            }
          }
      ...
      "ObjectNotFoundException should be thrown when" >> {
        ...
        "Getting the metadata of a non-existing object" in assertAllStagesStopped {
          nonExistingObj.metadata() must throwA(objectNotFound(nonExistingObj))
            .await(2, 5.seconds)
        }
        ...
        "Getting content of a non-existing object" in assertAllStagesStopped {
          get(nonExistingObj) must throwA(objectNotFound(nonExistingObj))
            .await(2, 5.seconds)
        }
      ...
      "BucketAlreadyExistsException should be thrown when" >> {
        "Creating an existing bucket" in assertAllStagesStopped {
          existingBucket.create(failsIfExists = true) must throwA(
            bucketAlreadyExists(existingBucket)
          ).await(2, 5.seconds)
        }
      }
    }
  ...
  def versioningErrorCommonTests(
    ...
    def vNonExistingBucket = nonExistingBucket.versioning
      .getOrElse(throw new IllegalArgumentException("versioning not supported"))

    def vExistingBucket = existingBucket.versioning
      .getOrElse(throw new IllegalArgumentException("versioning not supported"))
