...
lazy val playS3 = project
  ...
  .settings(
    name := "jinbe-s3-play-demo",
    libraryDependencies ++= Seq(
      guice,
      "com.typesafe.play" %% "play-ws-standalone" % "2.0.8",
      "io.github.cchantep" %% "jinbe-s3" % version.value,
      "io.github.cchantep" %% "jinbe-play" % playVer.value
    )
  )
...
lazy val playVfs = project
  ...
  .enablePlugins(PlayScala)
  .settings(
    name := "jinbe-vfs-play-demo",
    libraryDependencies ++= Seq(
      "com.typesafe.play" %% "play-ws-standalone" % "2.0.8",
      "io.github.cchantep" %% "jinbe-vfs" % version.value,
      "io.github.cchantep" %% "jinbe-play" % playVer.value
    )
  )
