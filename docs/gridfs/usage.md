# MongoDB GridFS with Jinbe

This guide covers using Jinbe with [MongoDB GridFS](https://docs.mongodb.com/manual/core/gridfs/), which provides a MongoDB-based object storage backend for distributed file storage.
...
```ocaml
libraryDependencies += "io.github.cchantep" %% "jinbe-gridfs" % "{{site.latest_release}}"
```
...
import io.github.cchantep.jinbe.{ Bucket, Object }
import io.github.cchantep.jinbe.gridfs.GridFSFactory
...
- `database` — Database name for storage (default: `jinbe`)
...
gridfs:mongodb://127.0.0.1:27017/jinbe
```
...
The tests will connect to `mongodb://localhost:27017/jinbe-test` by default. To use a different MongoDB instance, set the `MONGODB_TEST_URI` environment variable:

```bash
export MONGODB_TEST_URI=gridfs:mongodb://mongo-server:27017/jinbe-test
sbt gridfs/test
```
