# S3 Storage with Jinbe

This guide covers using Jinbe with Amazon S3 and S3-compatible services like CEPH and MinIO.
...
```ocaml
libraryDependencies += "io.github.cchantep" %% "jinbe-s3" % "{{site.latest_release}}"
...
import io.github.cchantep.jinbe.s3._
...
def sample1(implicit m: Materializer): Unit = {
  ...
  /* Get objects list */
  val objects: Future[List[io.github.cchantep.jinbe.Object]] = bucket.objects.collect[List]()
  objects.map(_.foreach(obj => println(s"- ${obj.name}")))
  
  /* Get object list with specified batch size, by default it 1000 */
  val allObjects: Future[List[io.github.cchantep.jinbe.Object]] = bucket.objects.withBatchSize(100).collect[List]()
