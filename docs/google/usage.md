# Google Cloud Storage with Jinbe

This guide covers using Jinbe with [Google Cloud Storage](https://cloud.google.com/products/storage/).
...
```ocaml
libraryDependencies += "io.github.cchantep" %% "jinbe-google" % "{{site.latest_release}}"
...
import io.github.cchantep.jinbe.google.{ GoogleStorage, GoogleTransport }
...
def sample1(implicit m: Materializer): Future[Unit] = {
  ...
  buckets.flatMap {
    _.headOption.fold(Future.successful(println("No found"))) { firstBucket =>
      ...
      val allObjects: Future[List[io.github.cchantep.jinbe.Object]] = bucketRef.objects.withBatchSize(100).collect[List]()
