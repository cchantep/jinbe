...
# curl -4 -D - -X POST -u '...' "https://ossrh-staging-api.central.sonatype.com/manual/upload/defaultRepository/io.github.cchantep"
...
if [ "x$SCALA_MODULES" = "x" ]; then
    SCALA_MODULES="core:jinbe-core s3:jinbe-s3 google:jinbe-google vfs:jinbe-vfs gridfs:jinbe-gridfs play:jinbe-play"
fi

SCALA_VERSIONS="2.11 2.12 2.13 3.4"
...
for V in $SCALA_VERSIONS; do
    ...
    for M in $SCALA_MODULES; do
        B=`echo "$M" | cut -d ':' -f 1`
        N=`echo "$M" | cut -d ':' -f 2`
        SCALA_DIR=$(compgen -G "target/out/jvm/scala-${V}*/${N}")
